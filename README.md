# 🚗 Tesla Global Delivery & Performance Analysis (2015–2025)
**By: Suman Saha — Data Analyst**

This project analyzes Tesla's delivery trends, regional performance, CO₂ savings, and financial insights from 2015 to 2025.  
Dashboard visualizations help understand how different Tesla models perform across regions and years.

---

## 📊 Project Overview
- Covers **5 Tesla models**: Model S, Model 3, Model X, Model Y, Cybertruck  
- Includes **11 years of data (2015–2025)**  
- Shows Tesla’s **global performance**, **market shifts**, and **environmental impact**
  
  ![img alt](https://github.com/iamsumansaha/Tesla-Global-Delivery-Performance-Analysi/blob/main/Dashboard/Screenshot%202025-12-11%20185214.png?raw=true)

---

## 🧩 Key Problems Identified
- Hard to compare deliveries across models  
- Regional delivery differences  
- Changing market demand  
- Gap between **target vs actual** deliveries  
- CO₂ impact varies by model  

---

## 📈 Performance Snapshot

| Metric | Value |
|--------|--------|
| **Total Deliveries** | **26.19M vehicles** |
| **YoY Growth** | **–1.6%** |
| **Avg CO₂ Saved** | **~750 tons** |
| **Avg Vehicle Value** | **$85K** |

---

## 🚘 Model-Wise Summary

### 🔵 Model S
- **Total deliveries:** 5.4M  
- **YoY growth:** –0.8%  
- **Peak year:** 2016  
- **CO₂ saved:** 403K tons  
- Strong demand in **North America** and **Europe**
  
![img alt](https://github.com/iamsumansaha/Tesla-Global-Delivery-Performance-Analysi/blob/main/Dashboard/Screenshot%202025-12-11%20185408.png?raw=true)

---

### 🟢 Model 3
- **Total deliveries:** 5.3M  
- **YoY growth:** +2.8%  
- **Peak year:** 2015  
- **CO₂ saved:** 403K tons  
- Top-performing in sustainability
  
![img alt](https://github.com/iamsumansaha/Tesla-Global-Delivery-Performance-Analysi/blob/main/Dashboard/Screenshot%202025-12-11%20185330.png?raw=true)

---

### 🟣 Model X
- **Total deliveries:** 5.2M  
- **YoY growth:** +0.76%  
- **Peak year:** 2022  
- **CO₂ saved:** 388K tons  
- Strong in **North America** & **Middle East**
  
![img alt](https://github.com/iamsumansaha/Tesla-Global-Delivery-Performance-Analysi/blob/main/Dashboard/Screenshot%202025-12-11%20185301.png?raw=true)

---

### 🟠 Model Y
- **Total deliveries:** 5.2M  
- **YoY growth:** –2.4%  
- **Peak years:** 2018 & 2022  
- **CO₂ saved:** 389K tons  
- High average vehicle value: **$86K**
  
![img alt](https://github.com/iamsumansaha/Tesla-Global-Delivery-Performance-Analysi/blob/main/Dashboard/Screenshot%202025-12-11%20185439.png?raw=true)

---

### 🔴 Cybertruck
- **Total deliveries:** 5.1M  
- **YoY growth:** –8.1%  
- **Peak year:** 2024  
- **CO₂ saved:** 382K tons  
- Strong presence in **North America**
  
![img alt](https://github.com/iamsumansaha/Tesla-Global-Delivery-Performance-Analysi/blob/main/Dashboard/Screenshot%202025-12-11%20185512.png?raw=true)

---

## 🌍 Regional Performance

### North America
- Largest and most consistent market  
- Highest deliveries every year  

### Europe
- Strong secondary market  
- Stable demand across models  

### Asia
- Mid-level performance  
- Consistent contribution  

### Middle East
- Smallest market  
- Growing year-by-year  

---

## 📉 Target vs Delivery Gap
- Tesla missed **350K–400K vehicles** on average  
- Main causes:
  - Production limits  
  - Supply chain issues  
  - Model-specific demand shifts  

---

## 🌱 Environmental Impact
- All Tesla models save **300K–400K tons** of CO₂ each  
- Strong contribution to global clean-energy goals  
- Marketing potential: **“Tesla = Greenest EV Brand”**

---

## 💰 Business Insights

| Model | Revenue |
|--------|------------|
| **Model S** | $460B |
| **Model Y** | $448B |
| **Model 3** | $447B |
| **Model X** | $432B |
| **Cybertruck** | $432B |

Model S generates the **highest revenue**.

---

## 🧠 Strategic Recommendations
- Increase production in **North America & Europe**  
- Improve Cybertruck performance  
- Expand charging infrastructure  
- Reduce target-delivery gap  
- Promote Tesla’s **CO₂ savings** in marketing  

---

## 📁 Project Files
- Power BI Dashboard  
- Dataset (2015–2025)  
- PPT Presentation (used for insights)

---

## 🛠️ Tech Stack
- **Power BI**  
- **Excel**  
- **Python** (Optional analytics)  

---

## 🙌 Author  
**Suman Saha**  
*Data Analyst — Passionate about Data Visualization, BI Dashboards & Analytics*

---

